  ...
  return (
    ...
    >
      {photos.length <= 0 ? (
        ...
        <ul className="relative w-full h-full bg-[#333] flex justify-center items-center">
          {stacks.map((photo, index) => {
            if (!rotationRef.current[photo.url]) {
              rotationRef.current[photo.url] = handleRotateImageCard();
            }
            return (
              <li
                key={photo.url + index}
                className={`absolute w-[70%] h-[70%] slide-from-left-animation`}
              >
                <div
                  style={{
                    transform: `rotate(${rotationRef.current[photo.url]}deg)`,
                  }}
                  className={`w-full h-full relative px-2 py-2 bg-white rounded-lg shadow-lg`}
                >
                  <img
